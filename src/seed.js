...
const TEST_ACCOUNTS = [
  ...
  {
    email: 'org@careconnect.com',
    password: 'Org@123456',
    fullName: 'منظمة رعاية',
    role: 'organization',
    organizationName: 'منظمة رعاية للتطوع'
  },
  {
    email: 'volunteer@careconnect.com',
    password: 'Vol@123456',
    fullName: 'متطوع تجريبي',
    role: 'volunteer'
  },
  {
    email: 'elderly@careconnect.com',
    password: 'Elderly@123456',
    fullName: 'أحمد المسن',
    role: 'elderly'
  }
];

function seedAccount(acc) {
  const emailKey = acc.email.trim().toLowerCase();
  if (store.get('credentials', emailKey)) return false;
...
function seedTestAccounts() {
  ...
  for (const acc of TEST_ACCOUNTS) {
    if (seedAccount(acc)) created++;
  }
  if (created > 0) {
    console.log(`[seed] Created ${created} test account(s).`);
  } else {
    console.log('[seed] All test accounts already exist.');
  }
}
